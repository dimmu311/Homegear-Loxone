...
namespace Loxone
{
    std::shared_ptr<LoxoneControl> createInstance::createInstanceFromTypeString(PVariable control, std::unordered_map<std::string, std::string> &room, std::unordered_map<std::string, std::string> &cat)
    {
        ...
        else if(type == "Irrigation") return std::make_shared<LoxoneControl>(control, room, cat, 0x405);
        ...
        else if(type == "NFCCodeTouch") return std::make_shared<NFCCodeTouch>(control, room, cat);
    ...
    std::shared_ptr<LoxoneControl> createInstance::createInstanceFromTypeNr(uint32_t typeNr, std::shared_ptr<BaseLib::Database::DataTable> rows)
    {
        ...
        else if(typeNr == 0x405) return std::make_shared<LoxoneControl>(rows, typeNr); //Irrigation
        ...
        else if(typeNr == 0x507) return std::make_shared<NFCCodeTouch>(rows, typeNr); //NFC Code Touch
